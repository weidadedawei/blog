---
layout:       post
title:        "kubernetes 命令行工具 kubectl 备忘单"
author:       "david"
catalog:      true
...
<img src="https://img-blog.csdnimg.cn/6428b253db9b4fe1a33ea9c10a3c435c.png" alt="在这里插入图片描述"  />
...
echo "source <(kubectl completion bash | sed s/kubectl/k/g)" >> ~/.bashrc # 在你的 bash shell 中永久地添加自动补全
echo "alias k=kubectl" >> ~/.bashrc #补全时为 `kubectl` 使用一个速记别名 k
source ~/.bashrc
```

如果没安装，先安装 bash 命令补全工具bash-completion安装
```bash
...
## --all-namespaces
用到 --all-namespaces 参数，简写：
```bash
kubectl get pods --all-namespaces
kubectl get pods -A
```

## 设置上下文默认命名空间namespace
...
```bash
kubectl config view 								 # 显示 kubeconfig 配置
